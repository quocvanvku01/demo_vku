...
<div class="all-post">
	<p class="ttkhoapost">Bài viết ở Khoa:</p>
	<select onchange="window.location = this.value;" id="khoapost">
		<option value="{{URL::to('all-post/all/1')}}">Tất cả</option>
		@foreach($list_khoa as $value_list_khoa)
		<option id="optionkhoa"  value="{{URL::to('all-post/'.$value_list_khoa->id_khoa.'/1')}}" <?php if($khtpost == $value_list_khoa->id_khoa) {echo "selected";} ?> >{{$value_list_khoa->name_khoa}}</option>
		@endforeach
	</select>
	<table cellpadding="0">
		<thead>
	...
	<div class="post-pagination">
		...
		@else
			...
			@else
			<p class="page-link"><a href="{{URL::to('all-post/'.$khtpost.'/'.$i)}}">{{$i}}</a></p>
			@endif
		@endif
		...
		<p class="page-link"><a href="{{URL::to('all-post/'.$khtpost.'/'.($currentPage+1))}}"><i class="fas fa-angle-right"></i></a></p>
		@else
